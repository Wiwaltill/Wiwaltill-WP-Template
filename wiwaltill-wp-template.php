<?php 
/**
 * Plugin Name: Wiwaltill WP Template
...
// Localization
/*function wiwaltill_wp_template_local_init(){
	$domain = 'custom-login';
	$locale = apply_filters('plugin_locale', get_locale(), $domain);
	load_textdomain($domain, WP_LANG_DIR.'/custom-css-for-login/'.$domain.'-'.$locale.'.mo');
	load_plugin_textdomain($domain, FALSE, dirname(plugin_basename(__FILE__)).'/languages/');
}
add_action('after_setup_theme', 'wiwaltill_wp_template_local_init');*/

//Auto Update
if( ! class_exists( 'Wiwaltill_WP_Template_UpdateChecker' ) ) {

	class Wiwaltill_WP_Template_UpdateChecker{
		...
		public function request(){
			...
			if( false === $remote || ! $this->cache_allowed ) {
				...
				$remote = wp_remote_get(
					'https://data.till-esser.de/wp-plugins/wiwaltill-wp-template/info.json',
					array(
						'timeout' => 10,
						'headers' => array(
	...
	new Wiwaltill_WP_Template_UpdateChecker();
